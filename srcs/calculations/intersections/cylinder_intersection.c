...
/*   Updated: 2025/10/06 09:16:23 by fkeitel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	is_intersection_inside(t_ray ray, t_object *obj, double t)
{
	t_vec3d	p;
	double	m;
...
static bool	is_intersection_on_cap(t_ray ray, t_object *obj, double t, int cap)
{
	t_vec3d	p;
	t_vec3d	v;
	double	dist;
...
static double	find_cylinder_side_intersection(t_ray ray, t_object *obj,
		double dot_dir_axis, double disc)
{
	...
	if (t[0] > 0 && is_intersection_inside(ray, obj, t[0]))
		return (t[0]);
	if (t[1] > 0 && is_intersection_inside(ray, obj, t[1]))
		return (t[1]);
	return (-1.0);
}
...
double	find_cylinder_cap_intersection(t_ray ray, t_object *obj, double dot_dir_axis)
{
	...
	if (fabs(denom) > 1e-6)
	{
		...
		if (t1 > 0 && is_intersection_on_cap(ray, obj, t1, 0))
			res = t1;
		else if (t2 > 0 && is_intersection_on_cap(ray, obj, t2, 1))
			res = fmin(res, t2);
	}
	return (res);
}
