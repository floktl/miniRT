...
/*   Updated: 2025/10/03 10:17:38 by mezhang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	double	dot_dir_axis_perp;
	double	dot_oc_axis;
	double	dot_oc_dir;

	double params[3]; // a, b, c
	dot_dir_axis_perp = vec_sub(ray_dir, vec_scale(obj->data.s_cylinder.axis,
				dot_dir_axis));
	dot_oc_axis = vec_dot(oc, obj->data.s_cylinder.axis);
	dot_oc_dir = vec_dot(oc, ray_dir);
	params[0] = vec_dot(ray_dir, ray_dir) - dot_dir_axis * dot_dir_axis;
	params[1] = 2 * (dot_oc_dir - dot_oc_axis * dot_dir_axis_perp);
		...
		double dot_dir_axis, double disc)
{
	...
	t1 = (-params[1] - sqrt(disc)) / (2 * params[0]);
	t2 = (-params[1] + sqrt(disc)) / (2 * params[0]);
	if (t1 > 0 && is_intersection_within_cylinder(ray, obj, t1, dot_dir_axis))
		return (t1);
	if (t2 > 0 && is_intersection_within_cylinder(ray, obj, t2, dot_dir_axis))
		return (t2);
	return (-1.0);
}
		...
		double dot_dir_axis)
{
	...
	if (fabs(denom) > 1e-6)
	{
		...
		t2 = (vec_dot(vec_add(obj->data.s_cylinder.base,
						vec_scale(obj->data.s_cylinder.axis,
